...
.stButton>button {
    background-color: #4B8BBE; color: white; font-weight: bold; border-radius: 8px; padding: 10px 20px;
}
.stButton>button:hover {
    background-color: #306998; color: #fff;
}
.port-table {border-collapse: collapse; width: 100%;}
.port-table th, .port-table td {border: 1px solid #ddd; padding: 8px;}
.port-table th {background-color: #4B8BBE; color: white; text-align: left;}
.open-port {color: green; font-weight: bold;}
.closed-port {color: red; font-weight: bold;}
</style>
            ...
            if target not in scan_result['scan']:
                ...
                if not ports_data:
                    ...
                    for port, info in sorted(ports_data.items()):
                        ...
                        <tr>
                            <td>{port}</td>
                            <td class="{ 'open-port' if state == 'open' else 'closed-port' }">{state}</td>
